...
app.post('/api/login', (req, res) => {
  ...
  db.query(query, [email], async (err, results) => {
    ...
    const token = jwt.sign({ id: user.id, email: user.email }, '너의 보안 jwt', { expiresIn: '1hr' });
...
app.get('/api/posts', (req, res) => {
  const query = 'SELECT id, title, content, author, DATE_FORMAT(created_at, "%Y-%m-%d %H:%i:%s") as created_at FROM posts ORDER BY created_at DESC';
...
app.post('/api/signup', async (req, res) => {
  ...
  db.query(checkQuery, [email], async (err, results) => {
    ...
    try {
      ...
      // 사용자 등록
      const insertQuery = 'INSERT INTO users (email, password, profile_image, join_data, is_verified) VALUES (?, ?, NULL, NOW(), false)';
      db.query(insertQuery, [email, hashedPassword], (err) => {
        ...
        res.status(201).send({ message: '회원가입이 완료되었습니다. 이메일을 확인해 주세요.' });
      });
    } catch (error) {
...
app.get('/api/verify-email', (req, res) => {
  ...
  try {
    ...
    db.query(updateQuery, [email], (err) => {
      ...
      res.send({ message: '이메일 인증이 완료되었습니다. 이제 로그인할 수 있습니다.' });
    });
  } catch (error) {

...
router.get("/search", authenticateToken, (req, res) => {
  ...
  db.query(sql, [`%${query}%`], (err, results) => {
    if (err) {
      return res
        .status(500)
        .send({ message: "DB 오류가 발생했습니다.", error: err });
    }
...
router.post("/send", authenticateToken, (req, res) => {
  ...
  const query =
    "INSERT INTO dm_direct_messages (sender_id, receiver_id, content) VALUES (?, ?, ?)";
  db.query(query, [senderEmail, receiverId, content], (err, result) => {
    if (err) {
      console.error("메시지 저장 중 오류 발생:", err);
      return res
        .status(500)
        .send({ message: "메시지 저장 중 오류가 발생했습니다.", error: err });
    }
    res.status(201).send({
...
// 대화 기록 조회
router.get("/:receiverEmail", authenticateToken, (req, res) => {
  ...
  db.query(
    ...
    (err, results) => {
      if (err) {
        ...
        return res
          .status(500)
          .send({ message: "대화 기록 조회 중 오류가 발생했습니다.", error: err });
      }
      res.send(results);
    }
  );
...
router.put("/read/:receiverId", authenticateToken, (req, res) => {
  ...
  db.query(query, [receiverId, senderId], (err, result) => {
    if (err) {
      return res
        .status(500)
        .send({ message: "읽음 상태 업데이트 중 오류가 발생했습니다.", error: err });
    }
